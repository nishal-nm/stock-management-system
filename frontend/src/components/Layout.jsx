...
import { LayoutDashboard, Package, PlusSquare, ArrowRightLeft, LogOut, FileText } from 'lucide-react';
...
export default function Layout() {
  ...
  const navItems = [
    ...
    { name: 'Create Product', path: '/products/new', icon: PlusSquare },
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 overflow-hidden font-sans">
      <aside className="w-72 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700/50 flex flex-col shadow-xl shadow-slate-200/20 dark:shadow-none z-10">
        <div className="p-8 flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white shadow-lg shadow-indigo-500/30 font-bold text-xl">
            V
          </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.path}
                className={clsx(
                  'group flex items-center gap-3.5 px-4 py-3 rounded-xl font-medium transition-all duration-300 relative overflow-hidden',
                  isActive 
              ...
              >
                ...
                <Icon size={20} className={clsx("relative z-10 transition-transform duration-300 group-hover:scale-110", isActive ? 'text-indigo-600 dark:text-indigo-400' : 'opacity-70')} />
